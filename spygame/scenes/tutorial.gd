...
func _process(delta: float) -> void:
	...
	if $"../war_map/Camera2D/ConfedMap/spy_centers6".intel_here == true and turn_3_pressed == false and tutorial_beats == 7:
	...
	if $"../war_map/Camera2D/ui_butts/upgrade".entered == true and fire_one == false:
		fire_one = true
...
func _input(event: InputEvent) -> void:
	...
	####i can fix all of this by just having this script in the overlay
	###
	###
	
	if event is InputEventMouseButton and event.pressed or event is InputEventScreenTouch and event.pressed:# event is InputEventScreenTouch and event.pressed or event.is_action_pressed("left_click"):
